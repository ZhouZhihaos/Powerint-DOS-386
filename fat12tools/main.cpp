...
void mkdir(char *dictname, int last_clust) {
  ...
  */
  ...
  time(&timep);
  p = gmtime(&timep);
  null.date = get_fat12_date(p->tm_year + 1900, p->tm_mon + 1, p->tm_mday);
...
void Copy(char *path, char *path1) {
  ...
  time(&timep);
  p = gmtime(&timep);
  finfo1->date = get_fat12_date(p->tm_year + 1900, p->tm_mon + 1, p->tm_mday);
...
void mkfile(char *name) {
  ...
  finfo->date = get_fat12_date(p->tm_year + 1900, p->tm_mon + 1, p->tm_mday);
  finfo->size = 0;
  return;
}
void cmd_dir() {
