...
char *_Asm(const char *asm_code1,int addr)
{
    ...
    //处理code
    char *asm_code = (char *)memman_alloc_4k(memman, 1000);
	int i;
	for (i = 0; i != 1000; i++) {
		asm_code[i] = 0;
	}
    ...
    if (asm_code1[t] == ';')
    {
		memman_free_4k(memman,asm_code,1000);
        return (char *)"\1";
    }
    kl = t;
    ...
    char *Result = (char *)memman_alloc_4k(memman, strlen(asm_code) + 1);
    if (strncmp(asm_code, "mov ", 4) == 0)
    {
        if (asm_code[4] == '$') //立即数
        {
            ...
            {
                printf("\nError!");
				memman_free_4k(memman,Result,strlen(asm_code)+1);
				memman_free_4k(memman,asm_code,1000);
                return 0;
            }
        }
