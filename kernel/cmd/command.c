...
static void HTTP_Socket_Handler(struct Socket *socket, void *base) {
  ...
  if (IsHttpGetHeader(data, size).ok) { // 是HTTP GetHeader
    ...
    if (strlen(IsHttpGetHeader(data, size).path) ==
        1) {            // 只有一个字符，那只能是"/"
      printf("root\n"); // 根目录
      HttpFile *f = FindForCount(1, httpFileList)->val; // 第一个文件就是根目录
      printf("f->path = %s\n", f->path);
      html_file = f->buf; // 设置html_file的地址
    } else {
      printf("Not root.\n"); // 不是根目录
  ...
  OK:
    ...
    unsigned char *packet = (unsigned char *)page_malloc(
        strlen(head) + strlen(html_file) + 1); // 声明最终的packet发送的数据
    memcpy((void *)packet, (void *)head, strlen(head)); // HTTP 标头
    memcpy((void *)(packet + strlen(head)), (void *)html_file,
           strlen(html_file)); // html文件数据
    packet[strlen(head) + strlen(html_file) + 1] =
        NULL; // 字符串结束符（为了下面调用的strlen函数）
    socket->Send(socket, packet, strlen(packet) + 1); // 调用Socket API发送
  } else {
