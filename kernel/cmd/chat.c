...
void chat_cmd() { // 用于接收用户想要发送的内容
  ...
  clean(chat_data, 2048); // 清空缓冲区
  socket->Send(socket, "CONNECT ", strlen("CONNECT ")); // 告诉服务器我们已经连接
  while (chat_flags) // 等待服务器回复
    ;
  uint8_t *dat = chat_data + sizeof(struct EthernetFrame_head) +
                 sizeof(struct IPV4Message) + sizeof(struct UDPMessage); // 获取数据
  if (strcmp(dat, "OK") != 0) { // 如果服务器是chat程序的正确服务端，那么将会返回OK字样
  ...
  for (;;) {
    ...
    socket->Send(socket, inp, strlen(inp)); // 通过socket API发送
  }
}
/* 声明用到的GUI控件 */
