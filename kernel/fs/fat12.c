...
static inline int get_fat12_date(unsigned short year, unsigned short month, unsigned short day) {
  year -= 1980;
...
struct FILEINFO *Get_File_Address(char *path1) {
  ...
  while (1) {
    ...
    finfo =
        dict_search(temp_name, (struct FILEINFO *)(ADR_DISKIMG + bmpDict), 224);
    if (finfo == 0) {
...
void changedict(char *dictname) {
  // cd命令的依赖函数
  struct FILEINFO *finfo =
      dict_search(dictname, (struct FILEINFO *)(ADR_DISKIMG + dictaddr), 224);
  //找文件夹
  if (finfo == 0) {
    //没找到
