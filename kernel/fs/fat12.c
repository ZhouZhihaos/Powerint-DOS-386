...
void mkdir(char *dictname, int last_clust) {
  ...
  */
  ...
  if (last_clust == 0) {
    ...
    for (int i = 1; FindForCount(i, drive_ctl.drives[drive_number].directory_clustno_list) != NULL; i++) {
      struct List *list = FindForCount(i, drive_ctl.drives[drive_number].directory_clustno_list);
