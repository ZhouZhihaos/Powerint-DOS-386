...
int jpeg_decode_huff(JPEG* jpeg,
                     int scan,
                     int* block,
...
void jpgview32(char* path, unsigned char* vram, int xsize) {
  ...
  struct RGB *picbuf, *q;
  picbuf = page_malloc(sizeof(struct RGB) * 1024 * 768);
  ...
  fclose(fp);
  page_free((int)picbuf, sizeof(struct RGB) * 1024 * 768);
...
void show_photo(char *path,vram_t *vr, int xsize) {
  ...
  for (i = 0; i < info[3]; i++) {
    q = picbuf + i * info[2];
    for (int j = 0; j < info[2]; j++) {
      color_t c = q[j].r << 16 | q[j].g << 8 | q[j].b;
      vr[i * xsize + j] = c;
    }
  }
  fclose(fp);
