...
void TCPProviderSend(uint32_t dstIP, uint32_t srcIP, uint16_t dstPort,
  ...
  uint8_t *dat = (uint8_t *)page_malloc(sizeof(struct TCPPesudoHeader) +
                                        sizeof(struct TCPMessage) + size);
  ...
  memcpy((void *)(tcp) + sizeof(struct TCPMessage), (void *)data, size);
  ...
  tcp->window = 0xffff;
  tcp->pointer = 0;
  tcp->options = (SYN == 1) ? 0xb4050402 : 0;
  tcp->checkSum = 0;
  tcp->checkSum =
      CheckSum((uint16_t *)dat, sizeof(struct TCPPesudoHeader) +
                                    sizeof(struct TCPMessage) + size);
  IPV4ProviderSend(6, IPParseMAC(dstIP), dstIP, srcIP, (uint8_t *)tcp,
                   sizeof(struct TCPMessage) + size, 0);
  page_free((void *)dat,
