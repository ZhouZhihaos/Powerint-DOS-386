...
void init_floppy()
{
    ...
    if (getbyte() == -1)
    {
        Panic_K("floppy: no floppy drive found");
        Panic_F("No fount FDC");
        return;
    }
    //设置软盘驱动器的中断服务程序
...
int getbyte()
{
    ...
    {
        msr = io_in8(FDC_MSR);
        if ((msr & 0xd0) == 0xd0) // 如果软盘控制器的状态寄存器的低五位是0xd0，说明我们能够从软盘DATA寄存器中读取
        {
            // 能读取了？那就读取吧，读完再返回回去
...
void wait_floppy_interrupt()
{
    ...
    while (!floppy_int_count)
        ;
    statsz = 0;                                           // 清空状态
    while ((statsz < 7) && (inportb(FDC_MSR) & (1 << 4))) //  状态寄存器的低四位是1（TRUE，所以这里不用写==），说明软盘驱动器没发送完所有的数据，当我们获取完所有的数据（状态变量=7），就可以跳出循环了
    {
...
int fdc_rw(int block, unsigned char *blockbuff, int read, unsigned long nosectors)
{
    ...
    unsigned char *p_tbaddr = (char *)0x80000; // 512byte 缓冲区（大家可以放心，我们再page.c已经把这里设置为占用了）
