...
struct SHTCTL *shtctl_init(unsigned char *vram, int xsize, int ysize)
{
	struct SHTCTL *ctl;
	int i;
	ctl = (struct SHTCTL *)page_kmalloc(sizeof(struct SHTCTL));
	if (ctl == 0) {
		goto err;
	}
	ctl->map = (unsigned char *)page_kmalloc(xsize * ysize);
	if (ctl->map == 0) {
		page_kfree((int) ctl, sizeof (struct SHTCTL));
		goto err;
	}
	ctl->vram = vram;
	ctl->xsize = xsize;
...
void ctl_free(struct SHTCTL *ctl)
{
	if (ctl != 0) {
		if (ctl->map != 0) {
			page_kfree((int) ctl->map, ctl->xsize * ctl->ysize);
		}
		page_kfree((int) ctl, sizeof (struct SHTCTL));
	}
}
