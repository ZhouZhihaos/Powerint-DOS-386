...
void init_gdtidt(void)
{
	...
	set_gatedesc(idt + 0x0a, (int)asm_error10, 2 * 8, AR_INTGATE32);      // 0x0a号异常
	set_gatedesc(idt + 0x0b, (int)asm_error11, 2 * 8, AR_INTGATE32);      // 0x0b号异常
	set_gatedesc(idt + 0x0c, (int)asm_error12, 2 * 8, AR_INTGATE32);      // 0x0c号异常
	set_gatedesc(idt + 0x0d, (int)asm_error13, 2 * 8, AR_INTGATE32);      // 0x0d号异常
	set_gatedesc(idt + 0x0e, (int)asm_error14, 2 * 8, AR_INTGATE32);      // 0x0e号异常
	set_gatedesc(idt + 0x10, (int)asm_error16, 2 * 8, AR_INTGATE32);      // 0x10号异常
	set_gatedesc(idt + 0x11, (int)asm_error17, 2 * 8, AR_INTGATE32);      // 0x11号异常
	set_gatedesc(idt + 0x12, (int)asm_error18, 2 * 8, AR_INTGATE32);      // 0x12号异常
	set_gatedesc(idt + 0x20, (int)asm_inthandler20, 2 * 8, AR_INTGATE32); // 计时器中断
	set_gatedesc(idt + 0x21, (int)asm_inthandler21, 2 * 8, AR_INTGATE32); // 键盘中断
	set_gatedesc(idt + 0x36, (int)asm_inthandler36, 2 * 8, AR_INTGATE32); // 系统API
