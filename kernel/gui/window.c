...
struct SHEET *MakeWindow(int x, int y, int w, int h, char *title,
  ...
  struct SHEET *sht = stl->sheets[stl->top - 1];
  uint32_t times = (sht->bxsize - 8) / (0xc6 - 0x84) + 1;
  for (int i = 3; i < 20; i++) {
    color_t color = COL_848484;
    for (int j = 3, count = 0; j < sht->bxsize - 4; j++, count++) {
      if (sht->buf[j + i * sht->bxsize] != COL_FFFFFF &&
          !(sht->bxsize - 37 <= j && j < sht->bxsize - 5 && 5 <= i && i < 19)) {
...
void make_window(vram_t *buf, int xsize, int ysize, char *title) {
  ...
  uint32_t times = (xsize - 8) / (240 - 106) + 1;
  for (int i = 3; i < 20; i++) {
