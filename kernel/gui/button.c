...
void Button_Draw(Button* buf) {
  ...
  putfonts_asc_sht(buf->buf, x + (w - strlen(buf->text) * 8) / 2, y + dy, COL_000000,
                    COL_C6C6C6, buf->text, strlen(buf->text));
  Draw_Line(buf->buf->buf, x + w - 1, y, x + w - 1, y + h, COL_848484,
              buf->buf->bxsize);
  Draw_Line(buf->buf->buf, x + w, y, x + w, y + h, COL_000000, buf->buf->bxsize);
  Draw_Line(buf->buf->buf, x, y, x + w, y, COL_FFFFFF, buf->buf->bxsize);
  Draw_Line(buf->buf->buf, x, y, x, y + h, COL_FFFFFF, buf->buf->bxsize);
  Draw_Line(buf->buf->buf, x, y + h, x + w, y + h, COL_000000, buf->buf->bxsize);
...
void Button_Draw_Clicking(Button* buf) {
  ...
  Draw_Line(buf->buf->buf, x, y + h, x + w, y + h, COL_FFFFFF, buf->buf->bxsize);
  Draw_Line(buf->buf->buf, x + w, y + h, x + w, y, COL_FFFFFF, buf->buf->bxsize);
  sheet_refresh(buf->buf, 0, 0, buf->buf->bxsize, buf->buf->bysize);
}
