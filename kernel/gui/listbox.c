...
static void Draw_All(listBox_t *listBox) {
  ...
  for (int i = listBox->now_min; i < listBox->now_max; i++) {
    //   printk("%08x %s\n", listBox->sheet->buf, listBox->item[i]);
    Sputs(listBox->sheet->buf, listBox->item[i], listBox->x, y, COL_000000,
          listBox->sheet->bxsize);
    y += 16;
  }
  refresh_all(listBox);
}
...
void init_listBox() { listBox_Node = NewList(); }
