    ...
    if (listBox->btns[i] == button_cl) {
      ...
      if (strcmp(listBox->item[i] + 17, "<FILE>") == 0) {
        ...
        if (strcmp(listBox->item[i] + 11, "BIN") == 0) {
          ...
          tmp[8 + strlen(name)] = '.';
          memcpy((void *)(tmp + 8 + strlen(name) + 1), (void *)(name + 8 + 3),
                 3);
...
void graphic(void) {
  ...
  // file mananger
  vram_t *buf_fm = (vram_t *)page_malloc(FMXSIZE * FMYSIZE * sizeof(color_t));
  struct SHEET *sht_fm = MakeWindow(200, 200, FMXSIZE, FMYSIZE, "文件管理器",
                                    shtctl, buf_fm, NULL);
