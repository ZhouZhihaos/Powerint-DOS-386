    ...
    if (header->type == swap16(IP_PROTOCOL)) { // IP数据报
      ...
      if (ipv4->version == 4) {
        if (ipv4->protocol == ICMP_PROTOCOL) { // ICMP
          icmp_handler(recvBufferDesc[currentRecvBuffer].address);
        } else if (ipv4->protocol == UDP_PROTOCOL) { // UDP
          udp_handler(recvBufferDesc[currentRecvBuffer].address);
        } else if (ipv4->protocol == TCP_PROTOCOL) { // TCP
          tcp_handler(recvBufferDesc[currentRecvBuffer].address);
        }
      }
...
void PCNET_IRQ(int *esp) {
  // printf("PCNET IRQ:%02x ",
  //        (0x20 + PCI_Get_Drive_IRQ_LINE(bus, dev, func)));
