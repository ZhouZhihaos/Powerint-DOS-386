...
void inthandler36(int edi,
  ...
  if (eax == 0x01) {
    ...
    if (running_mode == POWERDESKTOP) {
      ...
      int i, mx1 = NowTask()->mx, my1 = NowTask()->my, bufx = NowTask()->mx * 8,
             bufy = NowTask()->my * 16;
      int bx = mx1;
      int by = my1;
      int bmp = *(char*)(NowTask()->TTY->vram + by * 160 + bx * 2 + 1);
      mouse_ready(&mdec);
      for (;;) {
        ...
        if (fifo8_status(TaskGetMousefifo(task)) == 0) {
          ...
          // printk("mx = %d my = %d\n",NowTask()->mx,NowTask()->my);
          i = fifo8_get(TaskGetMousefifo(task));
          if (mouse_decode(&mdec, i) != 0) {
            if (NowTask()->TTY != now_tty() && NowTask()->TTY->using1 == 1) {
              continue;
            }
            if ((mdec.btn & 0x01) != 0) {
              intreturn(eax, ebx, NowTask()->mx, NowTask()->my, 1, edi, ebp);
              break;
            } else if ((mdec.btn & 0x02) != 0) {
              intreturn(eax, ebx, NowTask()->mx, NowTask()->my, 2, edi, ebp);
              break;
            } else if ((mdec.btn & 0x04) != 0) {
            ...
            *(char*)(0xb8000 + my1 * 160 + mx1 * 2 + 1) = bmp;
            bmp =
                *(char*)(0xb8000 + NowTask()->my * 160 + NowTask()->mx * 2 + 1);
            *(char*)(0xb8000 + NowTask()->my * 160 + NowTask()->mx * 2 + 1) =
                ~bmp;
            mouse_sleep(&mdec);
            sleep(50);
      ...
      mouse_sleep(&mdec);
      *(char*)(NowTask()->TTY->vram + NowTask()->my * 160 + NowTask()->mx * 2 +
               1) = bmp;
      NowTask()->mx = mx1;
      NowTask()->my = my1;
    }
