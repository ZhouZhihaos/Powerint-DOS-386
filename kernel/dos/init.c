...
void sysinit(void) {
  ...
  init_vdisk();
  init_mount_disk();
  SetDrive("FLOPPY_DRIVE");
  SetDrive("IDE_DRIVE");
  SetDrive("VDISK_DRIVE");
  SetDrive("NETCARD_DRIVE");
  normal.tss.ldtr = 0;
...
void Disk_Read(unsigned int lba, unsigned int number, void *buffer,
               char drive) {
  if (drive == 'A') {
    if (DriveSemaphoreTake(GetDriveCode("FLOPPY_DRIVE"))) {
      for (int i = 0; i != number; i++) {
        fdc_rw(lba + i, (unsigned char *)(buffer + i * 512), 1, 1);
      }
      DriveSemaphoreGive(GetDriveCode("FLOPPY_DRIVE"));
    }
  } else if (drive != 'B') {
    unsigned char drive1 = drive;
    if (DiskReady(drive1)) {
      if (DriveSemaphoreTake(GetDriveCode("IDE_DRIVE"))) {
        ide_read_sectors(drive1 - 'C', number, lba, 8, buffer);
        DriveSemaphoreGive(GetDriveCode("IDE_DRIVE"));
      }
    } else if (have_vdisk(drive1)) {
      if (DriveSemaphoreTake(GetDriveCode("VDISK_DRIVE"))) {
        rw_vdisk(drive1, lba, buffer, number, 1);
        DriveSemaphoreGive(GetDriveCode("VDISK_DRIVE"));
      }
    } else {
