		...
		if (ly > 0) {
			...
			if (ix < lx && lx < ix + 25 && iy <= ly && ly < iy + invline) {
				...
				if (*p != ' ') {
					...
					SDraw_Box(win, 19 * 8, 2 * 16, 19 * 8 + strlen(BMP) * 8, 3 * 16, 0, 336);
	...
	if (c != ' ') {
	  if ('a' <= c && c <= 'h') {
		...
		for (int j = 0; j < 16; j++, y+=1) {
		  if ((p[j] & 0x80) != 0) { SDraw_Px(win, x, y, col, 336); }
		  if ((p[j] & 0x40) != 0) { SDraw_Px(win, x+1, y, col, 336); }
		  if ((p[j] & 0x20) != 0) { SDraw_Px(win, x+2, y, col, 336); }
		  if ((p[j] & 0x10) != 0) { SDraw_Px(win, x+3, y, col, 336); }
		  if ((p[j] & 0x08) != 0) { SDraw_Px(win, x+4, y, col, 336); }
		  if ((p[j] & 0x04) != 0) { SDraw_Px(win, x+5, y, col, 336); }
		  if ((p[j] & 0x02) != 0) { SDraw_Px(win, x+6, y, col, 336); }
		  if ((p[j] & 0x01) != 0) { SDraw_Px(win, x+7, y, col, 336); }
		}
		y -= 16;
	  }
	}
