...
static unsigned int SectorBytes;
static unsigned int RootMaxFiles;
static unsigned int imgTotalSize;
static unsigned int Fat1Address;
static unsigned int Fat2Address;
static string _file;
static char *ptr_file;
static string ___file;
static int dictaddr;
static string View_path = "";
static int change_dict_times = 0;
...
void file_savefile(int clustno, int size, char *buf, int *fat,
  ...
  if (size > (clustall + 1) * ClustnoBytes) {
    ...
    int i;
    for (int size1 = clustall * ClustnoBytes, i = old_clustno; size1 > size;
         size1 -= ClustnoBytes) {
      fat[i] = 0;
      ff[i] = false;
...
void mkdir(char *dictname, int last_clust) {
  ...
  */
  ...
  time(&timep);
  p = gmtime(&timep);
  null.date = get_fat_date(p->tm_year + 1900, p->tm_mon + 1, p->tm_mday);
  null.time = get_fat_time(p->tm_hour, p->tm_min);
  for (int i = 0; i != 3072; i++) {
    if (!fat[i]) {
      null.clustno = i;
      fat[i] = 0xfff;
...
void Copy(char *path, char *path1) {
  ...
  time(&timep);
  p = gmtime(&timep);
  finfo1->date = get_fat_date(p->tm_year + 1900, p->tm_mon + 1, p->tm_mday);
  finfo1->time = get_fat_time(p->tm_hour, p->tm_min);
  return;
}
void del(char *cmdline) {
  //删除某个文件
...
void mkfile(char *name) {
  ...
  time(&timep);
  p = gmtime(&timep);
  finfo->time = get_fat_time(p->tm_hour, p->tm_min);
  finfo->date = get_fat_date(p->tm_year + 1900, p->tm_mon + 1, p->tm_mday);
  finfo->size = 0;
  return;
}
void cmd_dir() {
  ...
  if (argc == 1) {
    ...
    for (int i = 1; i < argc; i++) {
      if (strcmp(argv[i], "-cons") == 0) {
        ...
        FatClustnoFlags = (unsigned char *)malloc(3072 * sizeof(char));
        SectorBytes = *(unsigned short *)(ADR_DISKIMG + BPB_BytsPerSec);
        RootMaxFiles = *(unsigned short *)(ADR_DISKIMG + BPB_RootEntCnt);
        ClustnoBytes =
            SectorBytes * *(unsigned char *)(ADR_DISKIMG + BPB_SecPerClus);
        RootDictAddress = (*(unsigned char *)(ADR_DISKIMG + BPB_NumFATs) *
                               *(unsigned short *)(ADR_DISKIMG + BPB_FATSz16) +
                           *(unsigned short *)(ADR_DISKIMG + BPB_RsvdSecCnt)) *
                          SectorBytes;
        FileDataAddress = RootDictAddress + RootMaxFiles * 32;
        if (*(unsigned short *)(ADR_DISKIMG + BPB_TotSec16) != 0) {
          imgTotalSize =
              *(unsigned short *)(ADR_DISKIMG + BPB_TotSec16) * SectorBytes;
        } else {
          imgTotalSize =
              *(unsigned int *)(ADR_DISKIMG + BPB_TotSec32) * SectorBytes;
        }
        Fat1Address =
            *(unsigned short *)(ADR_DISKIMG + BPB_RsvdSecCnt) * SectorBytes;
        Fat2Address =
            Fat1Address +
            *(unsigned short *)(ADR_DISKIMG + BPB_FATSz16) * SectorBytes;
...
_Start:
  ...
  SectorBytes = *(unsigned short *)(ADR_DISKIMG + BPB_BytsPerSec);
  RootMaxFiles = *(unsigned short *)(ADR_DISKIMG + BPB_RootEntCnt);
  ClustnoBytes = SectorBytes * *(unsigned char *)(ADR_DISKIMG + BPB_SecPerClus);
  RootDictAddress = (*(unsigned char *)(ADR_DISKIMG + BPB_NumFATs) *
                         *(unsigned short *)(ADR_DISKIMG + BPB_FATSz16) +
                     *(unsigned short *)(ADR_DISKIMG + BPB_RsvdSecCnt)) *
                    SectorBytes;
  ...
  if (*(unsigned short *)(ADR_DISKIMG + BPB_TotSec16) != 0) {
    ...
    imgTotalSize = *(unsigned int *)(ADR_DISKIMG + BPB_TotSec32) * SectorBytes;
  }
  Fat1Address = *(unsigned short *)(ADR_DISKIMG + BPB_RsvdSecCnt) * SectorBytes;
